...
class ServerlessSDK {
  ...
  config(config = {}) {
    ...
    this.platformStage =
      process.env.SERVERLESS_PLATFORM_STAGE || config.platformStage || 'prod'
    config.context = config.context || {}
    this.context.orgUid = config.context.orgUid || this.context.orgUid || null
    this.context.orgName =
      config.context.orgName || this.context.orgName || null
    this.context.stageName =
      config.context.stageName || this.context.stageName || null
    this.context.appName =
      config.context.appName || this.context.appName || null
    this.context.instanceName =
      config.context.instanceName || this.context.instanceName || null
    this.context.componentName =
      config.context.componentName || this.context.componentName || null
  ...
  publish(event) {
    if (!this.isConnected()) {
      throw new Error(
        `You are not currently connected to the Serverless Platform.`
      )
    }
  ...
  async getComponent(componentName = null, componentVersion = null) {
    return await component.getFromRegistry(
      this,
      componentName,
      componentVersion
    )
  ...
  async getInstance(
    ...
    return await instance.getByName(
      this,
      orgName,
      stageName,
      appName,
