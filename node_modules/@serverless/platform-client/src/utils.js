...
if (typeof window === 'undefined') {
  ...
  const getDirSize = async (p) => {
    ...
    if (stat.isFile()) {
      ...
      return Promise.all(entries.map(e => getDirSize(path.join(p, e))))
   ...
   * @param {*} outputZipPath 
   */
  const zip = async(inputDirPath = null, outputFilePath = null, include = [], exclude = [], includeContent = []) => {
    ...
    zipper.addLocalFolder(inputDirPath)
    ...
    if (includeContent && includeContent.length) {
      includeContent.forEach((content) => zipper.addFile(content.fileName, Buffer.alloc(content.fileContent.length, content.fileContent)))
    }
  ...
  const unzip = async(inputZipPath, removeZip) => {
    ...
    if (removeZip) { fs.rmdirSync(inputZipPath) }
    return destPath
  }
