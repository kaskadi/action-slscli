...
if (typeof window === 'undefined') {
  ...
  const zip = async (
    ...
    if (exclude && exclude.length > 0) {
      const zipEntries = zipper.getEntries()
      const entriesToBeRemove = []
      for (const zipEntry of zipEntries) {
        for (const excludeItem of exclude) {
          if (minimatch(zipEntry.entryName, excludeItem, { nocase: true })) {
            entriesToBeRemove.push(zipEntry)
            break
          }
        }
      }
      // remove excluded entries
      for (const entryToBeRemove of entriesToBeRemove) {
        zipper.deleteFile(entryToBeRemove)
      }
    }
