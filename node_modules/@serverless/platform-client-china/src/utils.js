...
if (typeof window === 'undefined') {
  ...
  const zip = async (
    ...
    zipper.addLocalFolder(inputDirPath, null, (filePath) => {
      for (const excludeItem of exclude) {
        if (minimatch(filePath, excludeItem, { nocase: true, dot: true })) {
          return false;
        }
      }
      return true;
  ...
  const unzip = async (inputZipPath, removeZip) => {
    ...
    // Delete zip file to preserve space on Lambda (it's limited to 250MB)
    if (removeZip) {
