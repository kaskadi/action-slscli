...
const preRunSrcUpload = async (sdk, orgName, inputs) => {
  ...
  const size = await utils.getDirSize(packagePath);

  // lock deployment of code size greater than 200MB
  if (size > 200000000) {
    throw new Error(
      'Your code size must be less than 200MB.  Try using Webpack, Parcel or Lambda layers to reduce your code size.'
    );
  }
